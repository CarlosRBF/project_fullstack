...
import {
  ...
  mockedCustomerLogin,
  mockedContact,
  mockedContactEmailExist,
  mockedContactPhoneExist,
} from "../../mocks"
...
test("POST /customers - Musr be able to create a customer", async () => {
  ...
  expect(response.body.createdAt).toEqual(new Date("2022-02-04"))
  expect(response.body.isActive).toEqual(true)
  expect(response.status).toBe(201)
})

test("POST /users -  should not be able to create a customer that already exists", async () => {
  const response = await request(app).post("/customers").send(mockedCustomer)
...
test("DELETE /customers -  should not be able to list client not being authenticated", async () => {
  ...
  const response = await request(app).delete(
    `/customers/${UserTobeDeleted.body[0].id}`
  )
...
test("PATCH /customers - should not be able to update id field value",async () => {
    ...
    const admingLoginResponse = await request(app).post("customers/login").send(mockedCustomerLogin);
